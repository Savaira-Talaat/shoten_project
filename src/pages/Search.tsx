...
function buildActiveFiltersLabel(params: URLSearchParams) {
    ...
    const active = pairs.map(([k, v]) => [k, v.trim()] as const).filter(([, v]) => v.length > 0);
    ...
    <div style={styles.page}>
        ...
        {!hasAnyQuery && (
            <div style={styles.card}>
                ...
                Tape une recherche dans la barre en haut, ou utilise la recherche avancée.
                </div>
            </div>
        )}

        {hasAnyQuery && status === "success" && !error && (
            <div style={{ marginBottom: 14 }}>
                {label && <div style={styles.muted}>{label}</div>}
                <div style={styles.muted}>{(total ?? 0).toLocaleString()} résultat(s)</div>
            </div>
        )}
        ...
        {hasAnyQuery && status === "success" && !error && (docs?.length ?? 0) === 0 && (
            <div style={styles.card}>Aucun résultat.</div>
        )}

        {hasAnyQuery && status === "success" && !error && docs.length > 0 && (
